...
$ma_lhp = isset($_GET['MaLHP']) ? $_GET['MaLHP'] : null;
if ($ma_lhp) {
	
		$result_khoa = get_khoa_list($conn);
		$result_hocphan = get_hocphan_list($conn);
	// Truy vấn chi tiết 
		
		$course_info = get_course_info($conn, $ma_lhp);
		if ($course_info) {
		} else {
			echo "<p>Không tìm thấy học phần.</p>";
			exit();
		}
		
		$current_khoa_list = get_listcourse_info($conn, $ma_lhp);
}

	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update']) && isset($_POST["ma_lhp_update"]) && isset($_POST["selected_khoa"])) {
    // Lấy dữ liệu từ form
    $maLHP = $_POST['MaLHP']; 
    $tenHP = $_POST['TenHP'];
	$soTC = $_POST['SoTinChi'];
    ...
    $soTiet = $_POST['SoTiet'];
	$ma_lhp_update = $_POST["ma_lhp_update"];
    $selected_khoa = $_POST["selected_khoa"];
   

    // Gọi hàm cập nhật thông tin giảng viên
	$update_course_result = update_course_info($conn, $maLHP, $tenHP,  $soTC, $khoaPT, $hpTruoc, $soTiet);
    $update_list_course_result = update_list_course_info($conn, $ma_lhp_update, $selected_khoa);

   

    // Kiểm tra kết quả
    if ($update_course_result === true && $update_list_course_result === true) {
        header("Location: " . $_SERVER['PHP_SELF'] . "?MaLHP=" . urlencode($maLHP));
        exit(); 
    } else {
        echo "Lỗi cập nhật: ";
		if ($update_course_result !== true) {
            echo "Học phần - " . $update_course_result . "<br>";
        }
        if ($update_list_course_result !== true) {
            echo "Danh sách học phần - " . $update_list_course_result . "<br>";
        }
    }
	/*  if ($update_course_result === true ) {
        header("Location: " . $_SERVER['PHP_SELF'] . "?MaLHP=" . urlencode($maLHP));
	...
	<html lang="vn">
					...
					<form method="POST">
							...
							<table border="1">
								<tr>
									...
									<td>
										<input type="text" name="MaLHP" value="<?php echo htmlspecialchars($course_info['MaLHP']); ?>" readonly />
									</td>
								</tr>
								<tr>
									<td><strong>Tên học phần</strong></td>
									<td>
										<input type="text" name="TenHP" value="<?php echo htmlspecialchars($course_info['TenHP']); ?>" disabled />
									</td>
								</tr>
								<tr>
									<td><strong>Số tín chỉ</strong></td>
									<td>
										<select name="SoTinChi" id="SoTinChi" onchange="updateSoTiet()" disabled>
											<option value="2" <?php if ($course_info['SoTinChi'] == '2') echo 'selected'; ?> >2</option>
											<option value="3" <?php if ($course_info['SoTinChi'] == '3') echo 'selected'; ?>>3</option>
										</select>
									</td>
								</tr>
								<tr>
									...
									<td>
										<input type="text" name="SoTiet" id="SoTiet" value="<?php echo htmlspecialchars($course_info['SoTiet']); ?>" disabled />
									</td>
								</tr>
								<tr>
									<td><strong>Khoa phụ trách</strong></td>
									<td>
										<select name="KhoaPhuTrach" disabled>
										<?php
										while ($khoa = $result_khoa->fetch_assoc()) {
											echo "<option value='" . $khoa['MaKhoa'] . "' " . ($course_info['KhoaPhuTrach'] == $khoa['MaKhoa'] ? 'selected' : '') . ">" . htmlspecialchars($khoa['TenKhoa']) . "</option>";
										}
										?>
									</select>
									</td>
								...
								<tr>
									...
									<td>
									<select name="Hocphantruoc" disabled>
										<?php
										while ($hocphan = $result_hocphan->fetch_assoc()) {
											echo "<option value='" . $hocphan['MaLHP'] . "' " . ($course_info['Hocphantruoc'] == $hocphan['MaLHP'] ? 'selected' : '') . ">" . htmlspecialchars($hocphan['TenHP']) . "</option>";
										}
										?>
									</select>
									</td>
								...
								<tr style="display:none;">
									<td> <input type="checkbox" name="selected_courses[]" value="<?php echo htmlspecialchars($course_info['MaLHP']); ?> " checked> </td>
								</tr>
							</table>
							</div>
							<div>
									...
									<tr>
										<td>
											...
											if ($khoa_list->num_rows > 0) {
												echo "<input type='hidden' name='ma_lhp_update' value='" . htmlspecialchars($ma_lhp) . "'>";
												echo "<div class='khoa-list' style ='display: flex; flex-wrap: wrap; justify-content:space-between;'>";
												while ($khoa = $khoa_list->fetch_assoc()) {
													$checked = in_array($khoa["MaKhoa"], $current_khoa_list) ? "checked" : "";
													echo "<div class='khoa-item' style='  width: 48%; margin-bottom: 10px;  box-sizing: border-box;'>";
													echo "<input type='checkbox' name='selected_khoa[]' value='" . htmlspecialchars($khoa["MaKhoa"]) . "' $checked>";
		...
		if (backButton) {
			...
			function updateSoTiet() {
				const soTinChiSelect = document.getElementById('SoTinChi');
				const soTietInput = document.getElementById('SoTiet');

				// Get the selected value for credits
				const selectedSoTinChi = soTinChiSelect.value;

				// Update the periods based on the selected credits
				if (selectedSoTinChi === '2') {
					soTietInput.value = '36'; // 2 credits = 36 periods
				} else if (selectedSoTinChi === '3') {
					soTietInput.value = '45'; // 3 credits = 45 periods
				}
			}			
		</script>
		</body>
